...
profile.socials.forEach(s => {
    if (s.badge) readme += `[![${s.name}](https://img.shields.io/badge/${s.name.replace(' ', '_')}-${s.badge.color}?style=for-the-badge&logo=${s.badge.name}&logoColor=${s.badge.logoColor})](${s.url})\n`;  
});

readme += "\n## ✨ Languages and Frameworks I use\n\n";
profile.devSkills.forEach(s => {
    readme += `[![${s.name}](https://img.shields.io/badge/${s.name.replace(' ', '_')}-${s.badge.color}?style=for-the-badge&logo=${s.badge.name}&logoColor=${s.badge.logoColor})](#)\n`;
});
